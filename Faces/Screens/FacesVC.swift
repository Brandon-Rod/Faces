...
class FacesVC: UIViewController, NSFetchedResultsControllerDelegate {
    ...
    private func configureAddNewFaceButton() {
        
        view.addSubview(addNewFace)
        
        addNewFace.addTarget(self, action: #selector(addFaceButtonTapped), for: .touchUpInside)
        
        NSLayoutConstraint.activate([
        
            addNewFace.centerYAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -40),
            addNewFace.centerXAnchor.constraint(equalTo: view.trailingAnchor, constant: -40)
            
        ])
        
    }
    
    @objc private func addFaceButtonTapped() {

        let newFace = Face(context: dataController.container.viewContext)
        newFace.creationDate = Date()
        newFace.birthDay = Calendar.current.date(from: DateComponents(year: 2000, month: 4, day: 29))!
        newFace.sunSign = Strings.sunUnknown
        newFace.moonSign = Strings.moonUnknown
        newFace.risingSign = Strings.risingUnknown

        let destVC = FacesDetailVC(face: newFace, dataController: dataController)
...
extension FacesVC: UITableViewDataSource, UITableViewDelegate {
...
extension FacesVC: UISearchResultsUpdating {
